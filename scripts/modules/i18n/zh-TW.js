export const ZH_TW_TRANSLATIONS = {
    meta: {
        title: "Markdown 轉富文本",
        description: "即時將 Markdown 轉換並預覽為 HTML 富文本的工具。"
    },
    app: {
        title: "Markdown 編輯器"
    },
    language: {
        label: "語言"
    },
    controls: {
        clearContent: "清空內容",
        removeEmptyLines: "刪除空行",
        loadDefaultMarkdown: "載入預設 Markdown",
        pasteMode: "富文本貼上模式",
        pasteModeReplace: "取代",
        pasteModeAppend: "附加",
        pasteModePrepend: "前置",
        richTextFormat: "複製富文本格式",
        richTextFormatSop: "SOP手冊格式",
        richTextFormatPlain: "無格式",
        richTextFormatSlide169: "投影片 16:9",
        codeBlockToTable: "將程式碼語法轉換成單格表格",
        blankLineAfterTables: "表格後加入空行",
        removeHeadingBold: "移除標題中粗體的格式",
        slideFontSize: "字體大小",
        slideTableFontSize: "表格字體大小",
        slideTextFontSize: "其他文字字體大小",
        slideApplyTableColorsToText: "其他文字也套用表格的顏色",
        slideHeaderType: "Header 類型",
        slideHeaderTypeBothPrimary: "雙主",
        slideHeaderTypeColumnPrimary: "欄為主",
        slideHeaderTypeRowPrimary: "列為主",
        slideTableWidth: "表格寬度",
        slideTableWidthFull: "Full（960）",
        slideTableWidthHalf: "Half（450）",
        slideTableHeight: "表格高度",
        slideTableHeightFull: "Full（420）",
        slideTableHeightHalf: "Half（210）",
        slideTableHeightAuto: "Auto",
        sopSettings: "SOP 設定",
        renderSettings: "渲染設定",
        renderSettingsTitle: "渲染設定",
        sopTopHeadingHint: "若為 SOP 手冊，建議最頂層標題從 h2 開始。",
        sopSettingsTitle: "SOP 手冊格式設定",
        sopSettingsConfirm: "完成",
        close: "關閉",
        paragraphLineHeight: "段落行高",
        previewFont: "字型",
        previewFontNotoSansTc: "Noto Sans TC",
        previewFontMicrosoftJhengHei: "微軟正黑體",
        tableStyle: "表格格式",
        tableStyleGray: "灰",
        tableStyleBlue: "藍",
        tableStyleYellow: "黃",
        tableStyleRed: "紅",
        tableStyleGreen: "綠",
        tableStylePurple: "紫",
        tableStyleBrown: "褐",
        pasteRichText: "貼上富文本",
        copyRichText: "複製富文本",
        pasteCleanSettings: "清理富文本貼上格式",
        pasteCleanSettingsTitle: "清理富文本貼上格式",
        pasteCleanSettingsHint: "貼上剪貼簿富文本時，選擇要移除的格式。",
        pasteCleanFont: "清理字型與字級（含 font/span/div 標籤與 style）",
        pasteCleanColor: "清理文字與背景顏色",
        pasteCleanBold: "清理粗體",
        pasteCleanItalic: "清理斜體",
        pasteCleanLinks: "清理超連結",
        pasteCleanTableMetadata: "清理表格 metadata（xmlns、dir、data-*）",
        pasteCleanTableStyle: "清理表格樣式（border、cellpadding、cellspacing、width、bordercolor 等）",
        pasteCleanSelectAll: "全部勾選",
        pasteCleanDeselectAll: "全部取消"
    },
    panels: {
        markdownInput: "Markdown 輸入區",
        previewRichText: "即時預覽 (Rich Text)"
    },
    overlay: {
        dropMarkdownTitle: "放開滑鼠以載入 Markdown 檔案",
        dropMarkdownHint: "支援 .md 與 .markdown 檔案"
    },
    editor: {
        placeholder: "在此輸入或貼上 Markdown 內容...",
        topHeadingLevel: "最頂層標題"
    },
    toast: {
        copiedToClipboard: "已複製到剪貼簿！",
        richCopiedSuccess: "富文本已成功複製！",
        copyFailed: "複製失敗，請手動選取。",
        noEmptyLines: "沒有可刪除的空行。",
        removedEmptyLines: "已刪除空行。",
        defaultMarkdownLoaded: "已載入預設 Markdown。",
        clipboardEmpty: "剪貼簿沒有可貼上的內容。",
        clipboardBlank: "剪貼簿內容是空白，未更新內容。",
        pasteAndCopied: "富文本已以 {mode} 模式轉成 Markdown 並複製！",
        pasteCopyFailed: "富文本已以 {mode} 模式轉成 Markdown，但複製失敗。",
        clipboardPermissionDenied: "無法讀取剪貼簿，請確認瀏覽器權限。",
        onlyMarkdownFileAllowed: "僅支援拖曳 .md 或 .markdown 檔案。",
        markdownFileLoaded: "已載入 Markdown 檔案：{fileName}",
        markdownFileReadFailed: "讀取拖曳的 Markdown 檔案失敗。"
    },
    confirm: {
        clearAll: "確定要清空所有內容嗎？",
        loadDefaultMarkdown: "確定要以預設 Markdown 取代目前內容嗎？"
    },
    error: {
        loadDefaultContent: "無法讀取預設內容:"
    },
    fallback: {
        defaultMarkdownTitle: "# Markdown 編輯器"
    }
};
